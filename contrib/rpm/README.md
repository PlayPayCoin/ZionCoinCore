...
The RPM spec file provided here is for Zioncoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://Zioncoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Zioncoin git repository](https://github.com/Zioncoin/Zioncoin2) but are not part of
...
Currently only `Source10` is used, for the example `Zioncoin.conf` file.
...
in the Zioncoin git repository. At present only an SVG version of the Zioncoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the Zioncoin
...
needed when 0.12.1 is released, a proper fix is already in the Zioncoin git
master branch.
...
idea to build Zioncoin Core against a static version of that library compiled
...
The `Zioncoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `Zioncoin-server` package. At this point, most RPM based
...
The `Zioncoin-server` package should have SELinux support. How to properly do
...
Zioncoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/Zioncoin/). He has been
packaging Zioncoin for Fedora at least since 2012.
...
4. I renamed the `Zioncoin` package that contains the Qt GUI to `Zioncoin-core` as
that appears to be how the general population refers to it, in contrast to
`Zioncoin-xt` or `Zioncoin-classic`. I wanted to make sure the general population
