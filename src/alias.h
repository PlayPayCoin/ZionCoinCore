...
const int Zioncoin_TX_VERSION = 0x7400;
...
bool IsZioncoinTxMine(const CTransaction& tx,const std::string &type);
...
bool DecodeAndParseZioncoinTx(const CTransaction& tx, int& op, int& nOut, std::vector<std::vector<unsigned char> >& vvch);
...
CAmount convertCurrencyCodeToZioncoin(const std::vector<unsigned char> &vchAliasPeg, const std::vector<unsigned char> &vchCurrencyCode, const double &nPrice, const unsigned int &nHeight, int &precision);
...
CAmount convertZioncoinToCurrencyCode(const std::vector<unsigned char> &vchAliasPeg, const std::vector<unsigned char> &vchCurrencyCode, const CAmount &nPrice, const unsigned int &nHeight, int &precision);
...
std::string GenerateZioncoinGuid();
bool IsAliasOp(int op);
...
bool GetZioncoinData(const CTransaction &tx, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash, int& nOut);
bool GetZioncoinData(const CScript &scriptPubKey, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash);
...
bool IsZioncoinScript(const CScript& scriptPubKey, int &op, std::vector<std::vector<unsigned char> > &vvchArgs);
...
void CleanupZioncoinServiceDatabases(int &servicesCleaned);
int aliasunspent(const std::vector<unsigned char> &vchAlias, COutPoint& outpoint);
bool IsMyAlias(const CAliasIndex& alias);
void GetAddress(const CAliasIndex &alias, CZioncoinAddress* address, const uint32_t nPaymentOption=1);
void GetAddress(const CAliasIndex &alias, CZioncoinAddress* address, CScript& script, const uint32_t nPaymentOption=1);
