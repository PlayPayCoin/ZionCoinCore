...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"sysversion\": xxxxx,        (numeric) the client version\n"
            "  \"protocolversion\": xxxxx,   (numeric) the protocol version\n"
            "  \"walletversion\": xxxxx,     (numeric) the wallet version\n"
            "  \"balance\": xxxxxxx,         (numeric) the total Zioncoin balance of the wallet\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"Zioncoinaddress\"\n"
            "\nReturn information about the given Zioncoin address.\n"
            "\nArguments:\n"
            "1. \"Zioncoinaddress\"     (string, required) The Zioncoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"Zioncoinaddress\", (string) The Zioncoin address validated\n"
			// Zioncoin
			"  \"alias\" : \"Zioncoinalias\", (string) The Zioncoin alias (if any) corrosponding to this address\n"
    ...
    if (isValid)
    {
		...
		// Zioncoin v1 addy compatibility
...
#ifdef ENABLE_WALLET
        // Case 1: Zioncoin address and we have full public key:
        CZioncoinAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are Zioncoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) Zioncoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CZioncoinAddress address(innerID);

    UniValue result(UniValue::VOBJ);
	// Zioncoin v1 addy by default
    CTxDestination dest = address.Get();
	CZioncoinAddress v2addr;
	v2addr.Set(dest);
	CZioncoinAddress zaddr;
	zaddr.Set(dest, CChainParams::ADDRESS_ZEC);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"Zioncoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"Zioncoinaddress\"  (string, required) The Zioncoin address to use for the signature.\n"
